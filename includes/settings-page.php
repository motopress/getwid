...
class SettingsPage {
	...
	public function checkInstagramQueryURL() {
		...
		if ( isset( $_GET['getwid-instagram-success'] ) ) {
			add_action( 'admin_notices', array( $this, 'getwid_instagram_notice_success' ) );
		}

		if ( isset( $_GET['instagram-error'] ) ) {
			add_action( 'admin_notices', array( $this, 'getwid_instagram_notice_error' ) );
		}
	}
	...
	public function registerFields() {
		...
		add_settings_field(
			...
			__( 'Section Content Width', 'getwid' ),
			array( $this, 'renderSectionContentWidth' ),
			'getwid_appearance',
			'getwid_appearance'
		);
		register_setting(
		...
		add_settings_field(
			...
			__( 'Instagram Access Token', 'getwid' ),
			array( $this, 'renderInstagramToken' ),
			'getwid_general',
			'getwid_general'
		);
		register_setting(
		...
		add_settings_field(
			...
			__( 'Instagram Cache Timeout', 'getwid' ),
			array( $this, 'renderInstagramCacheTimeout' ),
			'getwid_general',
			'getwid_general'
		);
		register_setting(
		...
		add_settings_field(
			'getwid_contact_form_recipient_email',
			__( 'Recipient Email Address', 'getwid' ),
			array( $this, 'renderContactFormRecipientEmail' ),
			'getwid_general',
			'getwid_contact_from'
		);
		register_setting(
		...
		add_settings_field(
			...
			__( 'Recaptcha Site Key', 'getwid' ),
			array( $this, 'renderRecaptchaSiteKey' ),
			'getwid_general',
			'getwid_contact_from'
		);
		register_setting(
		...
		add_settings_field(
			...
			__( 'Recaptcha Secret Key', 'getwid' ),
			array( $this, 'renderRecaptchaSecretKey' ),
			'getwid_general',
			'getwid_contact_from'
		);
		register_setting(
		...
		add_settings_field(
			...
			array( $this, 'renderMailchimpApiKey' ),
			'getwid_general',
			'getwid_general'
		);
		register_setting(
		...
		add_settings_field(
			'getwid_disabled_blocks',
			__( 'Disable Getwid Blocks', 'getwid' ),
			array( $this, 'renderDisabledBlocks' ),
			'getwid_blocks',
			'getwid_blocks'
		);
	...
	public function renderInstagramToken() {
		...
		?>
					" class="button button-default"><?php echo esc_html__( 'Connect Instagram Account', 'getwid' ); ?></a>
		<?php
		if ( ! empty( $field_val ) ) {
			?>
			<a href="
			...
			?>
						" class="button button-default"><?php echo esc_html__( 'Refresh Access Token', 'getwid' ); ?></a>
			<?php
		}
		?>
		</p>
	...
	public function renderDisabledBlocks() {
		...
		$blocks         = getwid()->blocksManager()->getControlledBlocks();
	...
	public function renderAnimation() {
		...
		<label for="getwid_smooth_animation">
			<input type="checkbox" id="getwid_smooth_animation" name="getwid_smooth_animation" value="1"
			<?php
				checked( '1', $field_val );
			?>
			/>
