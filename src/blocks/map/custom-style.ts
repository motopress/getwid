...
function normalizeLegacyStyleLiteral( value: string ) {
	if (
		/(?:\bfunction\b|=>|\bwindow\b|\bdocument\b|\bthis\b|`|;)/.test( value )
	) {
		return null;
	}
...
export function parseCustomMapStyle( customStyle?: string ) {
	...
	const normalizedLegacyStyle =
		normalizeLegacyStyleLiteral( normalizedStyle );
