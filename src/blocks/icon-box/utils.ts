...
import type { ColorValue, IconBoxAttributes } from './types';

type WrapperStyleProps = {
	attributes: IconBoxAttributes;
	backgroundColor?: ColorValue;
	textColor?: ColorValue;
};

export function getBlockClassName( attributes: IconBoxAttributes ) {
	const { hoverAnimation, layout, textAlignment } = attributes;

	return {
		'getwid-animation': !! hoverAnimation,
		'has-icon-left': layout === 'left',
		'has-icon-right': layout === 'right',
...
export function getIconContainerClassName( attributes: IconBoxAttributes ) {
	const { iconPosition, iconStyle } = attributes;
