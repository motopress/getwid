...
import type {
	TemplatePostFeaturedBackgroundImageAttributes,
...
function getPaddingKey( device: PaddingDevice, side: PaddingSide ) {
	if ( device === 'desktop' ) {
		return `padding${ side }` as keyof TemplatePostFeaturedBackgroundImageAttributes;
	}

	const suffix = device === 'tablet' ? 'Tablet' : 'Mobile';
	return `padding${ side }${ suffix }` as keyof TemplatePostFeaturedBackgroundImageAttributes;
}

function getPaddingValueKey( side: PaddingSide ) {
	return `padding${ side }Value` as keyof TemplatePostFeaturedBackgroundImageAttributes;
}
	...
	return (
		<>
			<SelectControl
				...
				value={ value || '' }
				onChange={ ( nextValue ) => {
					const nextAttributes: Partial< TemplatePostFeaturedBackgroundImageAttributes > =
						{ [ key ]: nextValue };
...
function PaddingPanel( props: TemplatePostFeaturedBackgroundImageEditProps ) {
...
export default function Inspector(
	props: TemplatePostFeaturedBackgroundImageEditProps
) {
	const { attributes, setAttributes } = props;
	const {
		minHeight,
